...
void parse_csv(std::string filename, std::vector<Course> &courses)
{
  ...
  {
    std::vector<std::string> tempFields = split(tempLine, ',');
    tempCourse.title = tempFields[0];
    tempCourse.number_of_units = tempFields[1];
    tempCourse.quarter = tempFields[2];
...
void write_courses_offered(std::vector<Course> &all_courses)
{
  ...
  if (!ofs.is_open())
  {
    std::cerr << "Failed to open file: " << "student_output/courses_offered.csv" << std::endl;
    return;
  }
  ...
  // Create copy vector.
  std::vector<Course> newCourses = all_courses;

  // Find and output offered courses.
  for (Course elem : all_courses)
  {
    if (elem.quarter != "null")
    {
      delete_elem_from_vector(newCourses, elem);
      ofs << elem.title << "," << elem.number_of_units << "," << elem.quarter << '\n';
    }
  }

  // Replace original vector.
  all_courses = newCourses;
}
...
void write_courses_not_offered(std::vector<Course> unlisted_courses)
{
  ...
  if (!ofs.is_open())
  {
    std::cerr << "Failed to open file: " << "student_output/courses_not_offered.csv" << std::endl;
    return;
  }
